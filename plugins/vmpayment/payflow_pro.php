...
*
* @version $Id: payflow_pro.php 2225 2010-01-19 23:18:41Z rolandd $
* @package VirtueMart
* @subpackage payment
...
class plgPaymentPayflow_Pro extends vmPaymentPlugin {
	...
	function process_payment($order_number, $order_total, &$d) {
		...
		if( empty($transactionkey)) {
			...
			JApplication::enqueueMessage(JText::_('VM_PAYMENT_ERROR' . '. Technical Note: The required passwird is empty! The payment method settings must be reviewed.',false ), 'error');
			return false;
		}
	...
	function capture_payment( &$d ) {
		...
		if( !$db->next_record() ) {
			...
			JApplication::enqueueMessage('Error: Order not found.','error');
			return false;
		}
