...
class TransMenu {
	...
	function genMenuItem(&$row, $level, $pos){
		...
		if ($level){
			...
			echo "$pmenu.addItem(\"".htmlspecialchars($row->name,ENT_QUOTES)."\", \"$row->link\", $row->browserNav, $active);\n";
		}
		else{
			$pmenu = "ms";
		}
