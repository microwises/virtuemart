...
if (!empty($this->order['details']['BT']['coupon_code'])) {
    echo JText::_('COM_VIRTUEMART_COUPON_DISCOUNT') . ':' . $this->order['details']['BT']['coupon_code'] . ' ' . JText::_('COM_VIRTUEMART_PRICE') . ':' . $this->currency->priceDisplay($this->order['details']['BT']['coupon_discount']);
    echo "\n";
}
...
echo sprintf("%'-64.64s", '') . "\n";
// total order
echo JText::_('COM_VIRTUEMART_MAIL_SUBTOTAL_DISCOUNT_AMOUNT') . ' : ' . $this->currency->priceDisplay($this->order['details']['BT']['order_billDiscountAmount'])."\n";
