	...
	protected function getCarriers($_vendorId)
	{
		$_db = &JFactory::getDBO();
                if(version_compare(JVERSION,'1.6.0') < ''){
                        $_q = 'SELECT v.`virtuemart_shipping_carrier_id`   AS id '
...
//		$_currency = VirtueMartModelVendor::getCurrencyDisplay($_vendorId);
	...
	public function plgVmOnShowOrderShipperFE($_orderId)
	{
		...
		$_q = 'SELECT r.`shipping_rate_name` AS rate '
			...
			. 'WHERE r.`virtuemart_shipping_rate_id` = ' . $this->getShippingRateIDForOrder($_orderId) . ' '
...
//		}
	...
	protected function getShipperIDForOrder($_id)
	{
		...
		$_q = 'SELECT s.`shipping_rate_carrier_id` AS shipper_id '
			...
			. 'AND   o.`ship_method_id` = s.`virtuemart_shipping_rate_id`';
		$_db->setQuery($_q);
		if (!($_r = $_db->loadAssoc())) {
			return -1;
		}
	...
	protected function selectShippingRate(VirtueMartCart $_cart, $_shipperId = 0)
	{
		...
		$_db = &JFactory::getDBO();
		$_q = 'SELECT `virtuemart_shipping_rate_id` '
		...
		return $_r['virtuemart_shipping_rate_id'];
	}
	/**
	...
	final protected function selectedThisShipper($_selement, $_sid)
	{
		$_db = &JFactory::getDBO();
		  ...
		  if(version_compare(JVERSION,'1.6.0') < ''){
			...
			. ',    #__plugins AS j '
			. "WHERE vm.virtuemart_shipping_carrier_id = '$_sid' "
			...
			. ',      #__extensions    AS      j '
			. 'WHERE j.`folder` = "vmshipper" '
                        . "AND vm.virtuemart_shipping_carrier_id = '$_sid' "
	...
	final protected function getThisShipperName($_sid)
	{
		...
		$_q = 'SELECT `shipping_carrier_name` '
			...
			. "WHERE virtuemart_shipping_carrier_id ='$_pid' ";
		$_db->setQuery($_q);
		$_r = $_db->loadAssoc(); // TODO Error check
