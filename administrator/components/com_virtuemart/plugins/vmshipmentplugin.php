...
abstract class vmShipmentPlugin extends vmPSPlugin {
    ...
    public function plgVmOnSelectShipment(VirtueMartCart $cart, $selectedShipment = 0) {
	  ...
	  if ($this->getShipmentMethods($cart->vendorId) === false) {
	  return false;
	  }
	  $html = array();
...
/*
	...
	return parent::plgVmOnCheckAutomaticSelectedPlugin($cart, $cart_prices);
    }
    /**
...
// 		}
	...
	$html = '<table class="admintable">' . "\n"
		. '	<thead>' . "\n"
		. '		<tr>' . "\n"
		. '			<td class="key" style="text-align: center;" colspan="2">' . JText::_('COM_VIRTUEMART_ORDER_PRINT_SHIPMENT_LBL') . '</td>' . "\n"
		. '		</tr>' . "\n"
		. '	</thead>' . "\n"
		. '	<tr>' . "\n"
	...
	return parent::pluginSelected($virtuemart_shipmentmethod_id);
    }
...
// 			vmdebug('hm',$prod);
     ...
     */
    final protected function getShipmentMethod($shipment_id) {
	return parent::getPluginMethod($shipment_id);
    }
     ...
     * @author Oscar van Eijk
     * @author Max Milbers
     * @return True if the calling plugin has the given shipment ID
     */
    protected function getShipmentMethods($vendorId) {
	$this->shipments = parent::getPluginMethods($vendorId);
	...
	return parent::getThisPluginData($virtuemart_shipmentmethod_id);
    }
	...
	return parent::getPluginHtml($shipment, $selectedShipment, $shipmentSalesPrice);
    }
	...
	return parent::getPluginName($shipment);
    }

    /**
     * retrieve the Shipment name from the shipment order table
     * @param $order_number
     * @param $shipment_method_id
     * @return Shipment name
     */
    function getOrderShipmentName($order_number, $shipment_method_id) {
	return parent::getOrderPluginName($order_number, $shipment_method_id);
    }
