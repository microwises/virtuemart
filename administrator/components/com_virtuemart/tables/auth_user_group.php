 ...
 * @package	VirtueMart
 */
class TableAuth_user_group extends JTable
{
	...
	{
		parent::__construct('#__vm_auth_user_group', 'user_id', $db);
	}
	...
	{
    	if (!$this->user_id) {
			$this->setError(JText::_('Shopper must have an user id to set shoppergroup.'));
			dump($this->user_id,'table check no user id');
			return false;
		} else if (!$this->group_id) {
			$this->setError(JText::_('Shopper must have a group id to set shoppergroup'));
