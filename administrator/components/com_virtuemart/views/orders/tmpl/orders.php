...
if (count($this->orderslist) > 0) {
	...
	<?php
	/* Print view URL */
	$print_url = juri::root().'index.php?option=com_virtuemart&view=orders&layout=details&tmpl=component&virtuemart_order_id=' . $order->virtuemart_order_id . '&order_number=' .$order->order_number. '&order_pass=' .$order->order_pass;
	...
	$invoice_url = juri::root().'index.php?option=com_virtuemart&view=orders&layout=details&format=pdf&tmpl=component&virtuemart_order_id=' . $order->virtuemart_order_id . '&order_number=' .$order->order_number. '&order_pass=' .$order->order_pass;
	...
	$invoice_link .= '<span class="hasTip print_32" title="' . JText::_('COM_VIRTUEMART_INVOICE') . '">&nbsp;</span></a>';
