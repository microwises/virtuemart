...
//				dump($filename,'Media filepath ');
			...
			<tr>
				...
				?>
				<td><?php echo JHTML::_('link', JRoute::_($link), $productfile->file_name, array('title' => JText::_('COM_VIRTUEMART_EDIT').' '.$productfile->file_name)); ?></td>
				<!-- File role -->
				<td><?php
				...
				<?php
					if ($productfile->file_is_image) {
						...
						if (JFile::exists($fullimg) || (strtolower(substr($productfile->file_name, 0, 4)) == 'http')) {
							...
							echo JText::_('COM_VIRTUEMART_FILES_LIST_FULL_IMG').": ";
							echo JHTML::_('link', JURI::root().$productfile->file_url, JText::_('COM_VIRTUEMART_VIEW'), array('class' => 'modal', 'rel' => '{handler: \'iframe\', size: {x: '.$imgsize[0].', y: '.$imgsize[1].'}}'));
						}
						echo '<br />';
						...
						if (JFile::exists($thumbimg)) {
							...
							echo JText::_('COM_VIRTUEMART_FILES_LIST_THUMBNAIL_IMG');
							echo JHTML::_('link', JURI::root().str_ireplace(array(JPATH_SITE, '\\'), array('', '/'), $info['dirname']).'/resized/'.$basename, JText::_('COM_VIRTUEMART_VIEW'), array('class' => 'modal', 'rel' => '{handler: \'iframe\', size: {x: '.($imgsize[0]+20).', y: '.($imgsize[1]+20).'}}'));
						}
						else echo JText::_('COM_VIRTUEMART_THUMB_NOT_FOUND').': '.$thumbimg;
					}
				?>
				</td>
				<!-- File title -->
